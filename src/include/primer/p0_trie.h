...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (auto i : key) {
      if (!now->get()->HasChild(i)) {
        now->get()->InsertChildNode(i, std::make_unique<bustub::TrieNode>(TrieNode(i)));
        flag = true;
      }
      now = now->get()->GetChildNode(i);
    }
    if (!flag && now->get()->IsEndNode()) {  // 已存在末尾节点，且末尾结点是TrieNodeWithValue
      latch_.WUnlock();                      // 释放写锁
      return false;
    }

    // 末尾节点是TrieNode，或者是刚刚创建的
    *now = std::make_unique<TrieNodeWithValue<T>>(std::move(*(*now)), value);
    latch_.WUnlock();  // 释放写锁
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    latch_.RLock();
    std::vector<std::unique_ptr<bustub::TrieNode> *> trace;
    std::unique_ptr<TrieNode> *now = &root_;
    for (auto i : key) {
      if (!now->get()->HasChild(i)) {
        latch_.RUnlock();
    ...
    for (int i = trace.size() - 1; i >= 0; i--) {
      if (!now->get()->HasChildren() && (i == static_cast<int>(trace.size() - 1) || !now->get()->IsEndNode())) {
        trace[i]->get()->RemoveChildNode(key[i]);
        now = trace[i];
      } else {
