...
/* Tokens.  */
// 这个文件是一个 Bison 解析器的接口文件，用于定义解析器的令牌和语法树节点的类型
#ifndef YYTOKENTYPE
#define YYTOKENTYPE
/* Put the tokens into the symbol table, so that GDB and other debuggers
   know about them.  */
// 这些枚举定义了解析器的所有令牌类型，例如 IDENT、FCONST、SCONST 等
enum yytokentype {
  IDENT = 258,
  FCONST = 259,
  SCONST = 260,
...
// YYSTYPE 是一个联合体，定义了语法树节点的各种可能类型。
// 这些类型包括整数、字符串、布尔值以及各种 SQL 语法树节点类型（如 PGJoinType、PGDropBehavior 等）
...
// YYLTYPE 结构体，用于表示词法分析器和语法分析器中的位置信息
// 包含了四个整数字段，分别表示代码片段的起始行、起始列、结束行和结束列
// 这个结构体通常用于错误报告和调试信息中，以便准确定位代码中的问题
